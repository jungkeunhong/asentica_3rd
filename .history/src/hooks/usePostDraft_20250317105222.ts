...
// Debounce function specifically for the draft save function
function debounce(
  func: (draft: PostDraft) => void,
  wait: number
): (draft: PostDraft) => void {
  let timeout: NodeJS.Timeout | null = null;
  
  return function(draft: PostDraft) {
    if (timeout) clearTimeout(timeout);
    timeout = setTimeout(() => func(draft), wait);
  };
}
