    ...
    if (isOpen) {
      const fetchTreatmentOptions = async () => {
        try {
          ...
          const uniqueNames = [...new Set(nameData?.map(item => item.treatment_name))];
          ...
          setAvailableOptions({
            treatmentCategories: uniqueCategories as string[],
            efficacies: uniqueEfficacies as string[],
            treatmentNames: uniqueNames as string[]
          });
        } catch (error) {
