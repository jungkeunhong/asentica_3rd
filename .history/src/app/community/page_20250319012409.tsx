                  ...
                  <div className="p-2 max-h-64 overflow-y-auto">
                    <div className="flex flex-wrap gap-1">
                      {mockPopularTags.slice(0, 20).map((tag) => (
                        <Button key={tag.id} variant="ghost" size="sm" className={`text-xs rounded-full ${selectedTag === tag.name ? 'bg-primary/20 text-primary' : 'hover:bg-primary/10'}`} onClick={() => handleTagSelect(tag.name)}>#{tag.name}</Button>
                      ))}
                    </div>
                    <div className="mt-2 pt-2 border-t border-border">
                      <Link href="/tags" className="text-xs text-primary hover:underline">View all topics</Link>
                    </div>
                  </div>
                </div>
              )}
