...
export default function SearchContent({
  ...
  useEffect(() => {
    ...
    // 테스트를 위해 강제로 모달 표시 (개발 중에만 사용)
    if (!isLoggedIn && process.env.NODE_ENV === 'development') {
      console.log('개발 모드에서 로그인 모달 강제 표시');
      setTimeout(() => {
    ...
    const handleScroll = () => {
      ...
      // 스크롤 위치가 300px을 넘으면 모달 표시 (테스트를 위해 임계값 낮춤)
      if (!isLoggedIn && !scrollThreshold.current && window.scrollY > 300) {
  ...
  }, [isLoggedIn]); // isLoggedIn만 의존성으로 추가
      
      // 테스트를 위해 3초 후에 강제로 모달 표시 (개발 중에만 사용)
