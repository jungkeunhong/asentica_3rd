...
export default function SearchContent({
  ...
  const filteredMedspas = useMemo(() => {
    ...
    if (activeFilters.treatmentCategories && activeFilters.treatmentCategories.length > 0) {
      ...
      // Get medspa IDs that have the selected treatment categories
      const medspaIdsWithSelectedCategories = priceData
        .filter(price => {
          const match = price.treatment_category && 
            activeFilters.treatmentCategories?.includes(price.treatment_category);
          return match;
        })
        .map(price => price.medspa_name);
      
      console.log('Medspas with selected categories:', medspaIdsWithSelectedCategories);
      
      // Filter medspas by these IDs
      medspasCopy = medspasCopy.filter(medspa => 
        medspaIdsWithSelectedCategories.includes(medspa.medspa_name)
      );
    ...
    if (activeFilters.efficacies && activeFilters.efficacies.length > 0) {
      ...
      // Get medspa IDs that have the selected efficacies
      const medspaIdsWithSelectedEfficacies = priceData
        .filter(price => {
          const match = price.efficacy && 
            activeFilters.efficacies?.includes(price.efficacy);
          return match;
        })
        .map(price => price.medspa_name);
      
      console.log('Medspas with selected efficacies:', medspaIdsWithSelectedEfficacies);
      
      // Filter medspas by these IDs
      medspasCopy = medspasCopy.filter(medspa => 
        medspaIdsWithSelectedEfficacies.includes(medspa.medspa_name)
      );
    ...
    // 필터가 없거나 기본 케이스
    return medspasCopy;
  }, [medspas, selectedFilter, searchQuery, userLocation, findTreatmentPriceNumber, advancedFilters, activeFilters, priceData]);
