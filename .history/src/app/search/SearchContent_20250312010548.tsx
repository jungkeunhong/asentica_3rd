    ...
    if (activeFilters) {
      ...
      if (activeFilters.priceRange && activeFilters.priceRange.length === 2) {
        ...
        if (minPrice > 0 || maxPrice < 10000) {
          ...
          console.log(`Filtered to ${medspasCopy.length} medspas by price range`);
        }
      }
      
      // Filter by Google star ratings
      if (activeFilters.googleStars && activeFilters.googleStars.length > 0) {
        console.log(`Filtering by Google stars:`, activeFilters.googleStars);
        medspasCopy = medspasCopy.filter(medspa => {
          const rating = medspa.google_star || 0;
          // Check if the rating is greater than or equal to any of the selected minimum ratings
          return activeFilters.googleStars!.some(minRating => rating >= minRating);
        });
        console.log(`Filtered to ${medspasCopy.length} medspas by Google stars`);
      }
      
      // Filter by Yelp star ratings
      if (activeFilters.yelpStars && activeFilters.yelpStars.length > 0) {
        console.log(`Filtering by Yelp stars:`, activeFilters.yelpStars);
        medspasCopy = medspasCopy.filter(medspa => {
          const rating = medspa.yelp_star || 0;
          // Check if the rating is greater than or equal to any of the selected minimum ratings
          return activeFilters.yelpStars!.some(minRating => rating >= minRating);
        });
        console.log(`Filtered to ${medspasCopy.length} medspas by Yelp stars`);
      }
      
      // Filter by Google review count
      if (activeFilters.googleReviews) {
        console.log(`Filtering by Google review count: ${activeFilters.googleReviews}+`);
        medspasCopy = medspasCopy.filter(medspa => {
          const reviewCount = medspa.google_review || 0;
          return reviewCount >= activeFilters.googleReviews!;
        });
        console.log(`Filtered to ${medspasCopy.length} medspas by Google review count`);
      }
      
      // Filter by Yelp review count
      if (activeFilters.yelpReviews) {
        console.log(`Filtering by Yelp review count: ${activeFilters.yelpReviews}+`);
        medspasCopy = medspasCopy.filter(medspa => {
          const reviewCount = medspa.yelp_review || 0;
          return reviewCount >= activeFilters.yelpReviews!;
        });
        console.log(`Filtered to ${medspasCopy.length} medspas by Yelp review count`);
      }
