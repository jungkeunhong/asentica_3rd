...
export default function SearchContent({
  ...
  const filteredMedspas = useMemo(() => {
    ...
    // 필터가 없거나 기본 케이스
    return medspasCopy;
  }, [medspas, selectedFilter, searchQuery, userLocation, findTreatmentPriceNumber, advancedFilters, activeFilters]);
          ...
          <div className="container mx-auto pl-3 pb-2">
            <div className="flex items-center gap-2">
              ...
              <button 
                ...
                className="flex items-center justify-center px-3 py-1 border rounded-full text-sm"
              >
                Filters {activeFilters.villages && activeFilters.villages.length > 0 ? 
                  `(${activeFilters.villages.length})` : ''}
              </button>
            </div>
            {selectedFilter && (
              <div></div>
