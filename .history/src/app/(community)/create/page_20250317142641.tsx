  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      try {
        ...
        if (apiError.message.includes('User must be logged in')) {
          throw new Error('You must be logged in to publish a post. Please sign in and try again.');
        } else {
          console.error('API Error:', apiError);
          throw new Error(`Failed to save post: ${apiError.message || 'Unknown error'}`);
        }
      }
    } catch (error: any) {
      console.error('Error publishing post:', error);
      setPublishError(`Error publishing post: ${error.message || JSON.stringify(error)}`);
