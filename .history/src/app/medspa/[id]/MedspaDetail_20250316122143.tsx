...
export default function MedspaDetail({ medspa }: MedspaDetailProps) {
  ...
  useEffect(() => {
    ...
    const slideElements = sliderRef.current.querySelectorAll('.slide-item');
  ...
  // Handle phone call
  const handleCall = (phoneNumber: string | null, e: React.MouseEvent) => {
    e.preventDefault();
  ...
  // Replace the handleOpenMap function with this new version
  const handleOpenMap = (address: string, businessName: string) => {
    setMapAddress(address);
    setMapBusinessName(businessName);
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20 bg-white">
          ...
          >
            {imageArray.map((url, index) => (
              <div 
                key={index}
                data-index={index}
                className="w-full h-full flex-shrink-0 flex-grow-0 snap-center slide-item"
              >
        ...
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <div className="flex items-center gap-1">
            ...
            <a 
              ...
              className="text-xm text-gray-500 hover:text-amber-900"
            >
      ...
      <div className="px-4 mt-8">
        <h2 className="text-xl font-base mb-4 font-sans">Highlights</h2>
