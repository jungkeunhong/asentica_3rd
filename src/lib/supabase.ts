      ...
      try {
        ...
        try {
          ...
          if (authError) {
            ...
            // If we can't get auth session, just use mock data
            console.log('Falling back to mock data due to auth issue');
            return []; // Will trigger mock data use in the component
          } else {
          ...
          // Auth check failed, use mock data
          console.log('Falling back to mock data due to auth check exception');
          return []; // Will trigger mock data use in the component
        }
        ...
        if (error) {
          ...
          // Log error but don't throw, return empty array to trigger mock data
          console.log('Falling back to mock data due to query error');
          return [];
        }
        
        console.log('Successfully fetched posts:', data?.length || 0);
        // Only return data if we actually got some posts
        return (data && data.length > 0) ? data : [];
      } catch (queryError) {
        console.error('Error executing query:', queryError);
        // Query failed, silently use mock data
        console.log('Falling back to mock data due to query exception');
        return [];
      }
    } catch (error) {
