...
const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose, onLoginSuccess }) => {
    ...
    try {
      ...
      if (isSignUp) {
        ...
        if (data.user) {
          ...
          if (!data.user.email_confirmed_at) {
            setError('Email verification link has been sent. Please check your email.');
            return;
          }
      ...
      if (err instanceof Error) {
        ...
        if (err.message.includes('Invalid login credentials')) {
          ...
          setError('Email verification is not complete. Please check your email.');
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Google login error. Please try again later.');
    } finally {
