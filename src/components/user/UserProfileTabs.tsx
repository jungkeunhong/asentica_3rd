              ...
              <div className="flex items-center mb-3">
                <div className="flex items-center">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star 
                      key={i}
                      className={`h-4 w-4 ${i < (review.rating || 0) ? "text-amber-500 fill-amber-500" : "text-gray-300"}`}
                    />
                  ))}
                </div>
            ...
            <div key={comment.id} className="bg-white rounded-lg shadow p-4">
              <div className="mb-2">
                <p className="text-sm text-gray-500">
                  On <span className="font-medium text-gray-700">{comment.postTitle || 'Untitled Post'}</span>
                </p>
              </div>
          ...
          likes.map(like => (
            <div key={like.id} className="bg-white rounded-lg shadow p-4">
              ...
              <div className="text-right text-xs text-gray-500">
                {formatDistanceToNow(new Date(like.date || new Date()), { addSuffix: true })}
              </div>
            </div>
          ))
        ) : (
