...
export default function CommentSection({ 
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    try {
      // Use postId when adding a new comment
      await onAddComment(commentText, null);
      setCommentText('');
      
      // In a real app, we would fetch updated comments for this postId
  ...
  const handleSubmitReply = async (parentId: string) => {
    ...
    try {
      // Use postId when adding a reply
      await onAddComment(replyText, parentId);
      setReplyText('');
      setReplyingTo(null);
      
      // In a real app, we would fetch updated comments for this postId
      console.log(`Added reply to comment ${parentId} on post ${postId}`);
    } catch (error) {
